...
export class BaseChildren extends HTMLDivElement {
    ...
    constructor(template: HTMLTemplateElement, TAG_NAME: string) {
        ...
        this.observer = new MutationObserver((mutationList) => { 
            for (const mutation of mutationList) {
                ...
                } else if (mutation.type === "attributes" &&  mutation.target instanceof HTMLElement && mutation.attributeName && mutation.oldValue) {
                    const newValue = (mutation.target as HTMLElement).getAttribute(mutation.attributeName)!
                    if(mutation.oldValue !== newValue) this.#emitOnChildernAttrChange(mutation.target, mutation.attributeName, mutation.oldValue, newValue) 
                }
              }
        })
    ...
    childByName<T=HTMLElement>(name: string) { return this.children.namedItem(name) as T | null }

    #emitOnChildernChange() { 
        this.onChildernChange(this.children) 
    }
    #emitOnChildernAttrChange(target: HTMLElement, attributeName: string, oldValue: string, newValue: string) { this.onChildernAttrChange(target, attributeName, oldValue, newValue) }

    attributeChangedCallback(attrName: typeof BASE_CHILDREN_ATTRIBUTES[number], oldValue: string, newValue: string) {
        switch (attrName) {
            case 'id': this.idUpdate(oldValue, newValue)
                break
            default: this.attributeUpdate(attrName, oldValue, newValue)
        }
    }
    ...
    onChildernAttrChange (target: HTMLElement, attributeName:string, oldValue: string, newValue: string) { }
