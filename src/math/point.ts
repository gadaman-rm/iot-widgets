...
export const cmpPoint = (p1: Point, p2: Point) =>
  p1.x === p2.x && p1.y === p2.y
    ? CmpPoint.EQUAL
    : p1.x > p2.x && p1.y > p2.y
      ? CmpPoint.MORE
...
export const absPoint = (point: Point) =>
  Object.freeze({ x: Math.abs(point.x), y: Math.abs(point.y) })
export const addPoint = (p2: Point, p1: Point) =>
  Object.freeze({ x: p2.x + p1.x, y: p2.y + p1.y })
export const subPoint = (p2: Point, p1: Point) =>
  Object.freeze({ x: p2.x - p1.x, y: p2.y - p1.y })
export const divPoint = (p2: Point, p1: Point) =>
  Object.freeze({ x: p2.x / p1.x, y: p2.y / p1.y })
export const mulPoint = (p2: Point, p1: Point) =>
  Object.freeze({ x: p2.x * p1.x, y: p2.y * p1.y })
