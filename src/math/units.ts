...
export const emToPX = (em: number, target: HTMLElement) => em * parseFloat(getComputedStyle(target.parentElement!).fontSize)
export const pxToEM = (px: number, target: HTMLElement) => px / parseFloat(getComputedStyle(target.parentElement!).fontSize)
...
export const convert_units = (() => {
    ...
    return (val: number, from: keyof typeof modes, to: keyof typeof modes, context = document.body) => {
        if (context)
