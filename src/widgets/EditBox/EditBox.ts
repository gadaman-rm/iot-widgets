...
export class EditBox extends BaseSvg {
  ...
  constructor(svgContainer: SvgContainer | undefined = undefined) {
    ...
    this.render()
    this.editStartEvent = new CustomEvent<{ type: EditEvent["type"] }>(
      "edit-start",
      { detail: { type: "" } as any },
    )
    this.editEvent = new CustomEvent<EditEvent>("edit", { detail: {} as any })
    this.editEndEvent = new CustomEvent<{ type: EditEvent["type"] }>(
      "edit-end",
      { detail: { type: "" } as any },
    )
  }

  public set svgContainer(svgContainer: SvgContainer) {
    this.#svgContainer = svgContainer
  }
  public get svgContainer(): SvgContainer | undefined {
  ...
  toTransformBox = (box: {
    ...
    const { tl, tr, br, bl, aabox } = toTransformBox(
      box.x,
      box.y,
      box.width,
      box.height,
  ...
  render() {
    if (
      ...
      this.bmidResizeRef.setAttribute(
        "x",
        (this.width * 0.5 - this.controllerSize / 2).toString(),
      )
      this.bmidResizeRef.setAttribute(
        "y",
        (this.height - this.controllerSize / 2).toString(),
      )
      ...
      this.brResizeRef.setAttribute(
        "x",
        (this.width - this.controllerSize / 2).toString(),
      )
      this.brResizeRef.setAttribute(
        "y",
        (this.height - this.controllerSize / 2).toString(),
      ...
      this.rmidResizeRef.setAttribute(
        "x",
        (this.width - this.controllerSize / 2).toString(),
      )
      this.rmidResizeRef.setAttribute(
        "y",
        (this.height * 0.5 - this.controllerSize / 2).toString(),
      )
  ...
  addEventListener(
    type: "edit-start",
    listener: (e: { detail: { type: EditEvent["type"] } }) => void,
    options?: boolean | AddEventListenerOptions | undefined,
  ): void
  addEventListener(
    type: "edit",
    listener: (e: { detail: EditEvent }) => void,
    options?: boolean | AddEventListenerOptions | undefined,
  ): void
  addEventListener(
    type: "edit-end",
