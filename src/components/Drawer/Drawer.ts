...
export class Drawer extends HTMLDivElement {
  ...
  constructor() {
    ...
    this.initHandler()
    setTimeout(() => {
      if (getCssVar("--drawer-animation", this) === "show")
  ...
  public get onClose(): string {
    return this.getAttribute("onclose")!
  }
  public set onClose(fn: () => void) {
    this.#onClose = fn
  }

  attributeChangedCallback(
    attrName: (typeof DRAWER_ATTRIBUTES)[number],
    oldValue: string,
    newValue: string,
  ) {
    switch (attrName) {
      ...
      case "size":
        ...
        break
      case "offset":
        this.offsetUpdate(+oldValue, +newValue)
        break
      case "anchor":
  ...
  sizeUpdate(oldsize: number, newSize: number) {
    ...
    if (
      ...
      newSize &&
      oldsize !== newSize &&
      !this.validateAndSetSize(newSize)
    )
      this.size = oldsize
  }
